...
def create_card():
    ...
    card_limit = request.form.get("card_limit", "").strip()
...
def card_action(card_id):
    ...
    used_limit, reserved_limit, card_limit = float(card["used_limit"]), float(card["reserved_limit"]), float(card["card_limit"])
